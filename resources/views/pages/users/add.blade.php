                                    ...
                                    <div class="card-body">
                                        <form @if(isset($user->id)) action="{{ route('admin.user.update',$user->id) }}" @else action="{{ route('admin.user.store') }}"" @endif method="post">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control @error('mobile') is-invalid @enderror" id="mobile" name="mobile" value="@if(isset($user->id)) {{ $user->mobile }} @elseif(old('mobile')) {{ old('mobile') }} @endif" required>
                                                @error('mobile')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
